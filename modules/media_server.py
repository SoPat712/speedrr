...
class MediaServerModule:
    ...
    def get_reduction_value(self) -> tuple[float, float]:
        ...
        if stream_based_servers:
            ...
            # We use a special marker (negative infinity) to signal stream-based mode to main loop
            return float('-inf'), 0
    ...
    def get_target_upload_speed(self) -> Union[int, float, str]:
        ...
        # Find the first server with stream-based speeds enabled
...
class BaseServer(threading.Thread):
    def __init__(self, config: SpeedrrConfig, server_config: MediaServerConfig, module: MediaServerModule) -> None:
        ...
        self._module.stream_count_dict[self._server_config] = 0
